...
final class GhosttyCallbacks: @unchecked Sendable {
    ...
    func action(target: ghostty_target_s, action: ghostty_action_s) -> Bool {
        ...
        case GHOSTTY_ACTION_COLOR_CHANGE:
            ...
            // Only background/foreground are reported to the window chrome;
            // palette/cursor color changes don't need a window repaint.
            guard change.kind == GHOSTTY_ACTION_COLOR_KIND_BACKGROUND
                    || change.kind == GHOSTTY_ACTION_COLOR_KIND_FOREGROUND
            else { return true }
            let nsColor = NSColor(
            ...
            DispatchQueue.main.async {
                GhosttyApp.shared.reportResolvedColor(kind: kind, nsColor: nsColor)
            }
            return true
        default:
            return false
